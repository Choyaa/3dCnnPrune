...
#################tttttttttttttttrtttttt
    with torch.no_grad():
        for i, (inp, targets, input_length) in enumerate(data_loader):
            ...
            for j in range(0, step+1):         
                ...
                if len(M) < 8:
                    ...
                    input_single = inp[:,:,M[0]:(M[0]+opt.sample_duration),:,:]
                    input_single =  input_single.numpy()      #######tensor ------  ndarray
                    inputs[0].host = input_single.reshape(-1)   #hostttthhhhh
                    trt_outputs = do_inference(context, bindings=bindings, inputs=inputs, outputs=outputs, stream=stream)
                    output_trt = postprocess_the_outputs(trt_outputs[0], shape_of_output).squeeze()
                    output_trt = torch.from_numpy(output_trt)    #######ndarray ------ tensor
                    detector.append(output_trt.softmax(0).data)
                    if not SoG:
