           ...
           # device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
            ...
            for j in range(0, step+1):
                ...
                if len(M) < 8:
                    ...
                    output_single = torch.squeeze(model(input_single)).cpu()
 ...
 #               print(input_viterbi,file =f)
  #              f.close 
    ...
    ######  monitor the error changes for every tuple
            print('tuple: {0}, processing time: {batch_time.avg:.5f}, detecotr_error: {1}, order_error: {2},path_m:{3},targets:{4}'.format(i, detector_error, order_error,targets,path_m, batch_time=batch_time))
